...
function ExportConfirmModal({ isOpen, onClose, onConfirm, totalData, filterName, isExporting }) {
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 20px 45px rgba(0,0,0,0.18)", border: "1px solid #e5e7eb",
      ...
      >
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: "#111827", margin: 0, letterSpacing: "-.3px" }}>Konfirmasi Export PDF</h3>
            <p style={{ fontSize: 12, color: "#6b7280", margin: "3px 0 0 0" }}>Unduh laporan riwayat pengecekan & foto</p>
          </div>
        </div>

        {/* Content */}
        <div style={{ padding: "20px 24px" }}>
          <p style={{ fontSize: 13, color: "#374151", margin: "0 0 14px 0", lineHeight: 1.5 }}>
            Apakah Anda yakin ingin mengunduh laporan PDF riwayat pengecekan energi?
          </p>
          ...
          <div style={{ background: "#f8fafc", borderRadius: 12, border: "1px solid #e2e8f0", padding: "12px 16px", display: "grid", gap: 8, fontSize: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <strong style={{ color: "#0f172a" }}>{Math.min(totalData, 100)} Pengecekan</strong>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#64748b" }}>Filter Kondisi:</span>
              <strong style={{ color: "#16a34a" }}>{filterName}</strong>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#64748b" }}>Lampiran Foto:</span>
              <strong style={{ color: "#2563eb" }}>Foto Hasil Deteksi (Embedded)</strong>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#64748b" }}>Format File:</span>
              <strong style={{ color: "#0f172a" }}>PDF Document (.pdf)</strong>
            </div>
          </div>
        </div>

        {/* Footer Actions */}
        <div style={{ padding: "14px 24px", background: "#f8fafc", borderTop: "1px solid #f1f5f9", display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button
            onClick={onClose}
            ...
            style={{
              ...
              cursor: isExporting ? "not-allowed" : "pointer", fontFamily: "inherit",
              transition: "all .15s",
            }}
          >
            Batal
          ...
          <button
            ...
            style={{
              ...
              background: isExporting ? "#15803d" : "#16a34a", color: "#fff",
              fontSize: 12, fontWeight: 700, cursor: isExporting ? "not-allowed" : "pointer",
              fontFamily: "inherit", boxShadow: "0 4px 14px rgba(22, 163, 74, 0.25)",
      ...
      <style>{`@keyframes fadeIn{from{opacity:0;transform:scale(.96)}to{opacity:1;transform:scale(1)}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    </div>
  );
}
        ...
        <div style={{ background: "#fff", borderRadius: 11, border: "1px solid #e5e7eb", padding: "11px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", boxShadow: "0 1px 3px rgba(0,0,0,.04)" }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none", borderRadius: 7, background: "#16a34a",
