...
const INIT_USERS = [
  { id:1, nama:"Admin Utama", email:"admin@hematrix.com",     role:"Administrator", status:"Aktif",       loginTerakhir:"01 Mei 2026 09:30", dibuat:"15 Jan 2026" },
  { id:2, nama:"Operator 1",  email:"operator1@hematrix.com", role:"Operator",      status:"Aktif",       loginTerakhir:"30 Apr 2026 16:45", dibuat:"20 Jan 2026" },
  { id:3, nama:"Viewer 1",    email:"viewer1@hematrix.com",   role:"Viewer",        status:"Aktif",       loginTerakhir:"28 Apr 2026 10:20", dibuat:"25 Jan 2026" },
  { id:4, nama:"Operator 2",  email:"operator2@hematrix.com", role:"Operator",      status:"Tidak Aktif", loginTerakhir:"15 Apr 2026 14:00", dibuat:"01 Feb 2026" },
];

export default function PenggunaSystem() {
  const [users,   setUsers]   = useState(INIT_USERS);
  const [showAdd, setShowAdd] = useState(false);
  const [form,    setForm]    = useState({ nama:"", email:"", role:"Viewer" });
  ...
  const handleAdd = () => {
    ...
    setUsers(u => [...u, { id: Date.now(), ...form, status:"Aktif", loginTerakhir:"–", dibuat: new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"}) }]);
    setForm({ nama:"", email:"", role:"Viewer" });
    setShowAdd(false);
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        {[
          ...
          { label:"Operator",        value:totalOperator, iconBg:"#dbeafe", iconColor:"#2563eb" },
      ...
      <div style={{ background:"#fff", borderRadius:11, border:"1px solid #e5e7eb", padding:"14px 18px", boxShadow:"0 1px 3px rgba(0,0,0,.05)" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
          {[
            ...
            { role:"Operator",      color:"#2563eb", items:["Monitoring dan pengecekan","Kelola perangkat IoT","Lihat riwayat dan grafik"] },
      ...
      <div style={{ background:"#fff", borderRadius:11, border:"1px solid #e5e7eb", boxShadow:"0 1px 3px rgba(0,0,0,.05)", overflow:"hidden" }}>
        <div style={{ padding:"13px 18px", borderBottom:"1px solid #f3f4f6", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <button onClick={() => setShowAdd(v=>!v)} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:"#16a34a", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
            + Tambah Pengguna
          </button>
        </div>
        ...
        {showAdd && (
          <div style={{ padding:"12px 18px", borderBottom:"1px solid #f3f4f6", background:"#f9fafb", display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
            {[{label:"Nama",key:"nama",type:"text"},{label:"Email",key:"email",type:"email"}].map(({ label, key, type }) => (
              <div key={key} style={{ flex:"1 1 150px" }}>
                <div style={{ fontSize:11, fontWeight:600, color:"#374151", marginBottom:4 }}>{label}</div>
                <input type={type} value={form[key]} onChange={e => setForm(f=>({...f,[key]:e.target.value}))} style={{ width:"100%", padding:"7px 10px", border:"1px solid #e5e7eb", borderRadius:7, fontSize:12, fontFamily:"inherit", outline:"none" }} />
              </div>
            ))}
            <div style={{ flex:"0 0 110px" }}>
              ...
              <select value={form.role} onChange={e => setForm(f=>({...f,role:e.target.value}))} style={{ width:"100%", padding:"7px 10px", border:"1px solid #e5e7eb", borderRadius:7, fontSize:12, fontFamily:"inherit", outline:"none" }}>
                {["Administrator","Operator","Viewer"].map(r=><option key={r}>{r}</option>)}
              </select>
            </div>
            <div style={{ display:"flex", gap:7 }}>
              <button onClick={handleAdd} style={{ padding:"7px 14px", borderRadius:7, border:"none", background:"#16a34a", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>Simpan</button>
              <button onClick={()=>setShowAdd(false)} style={{ padding:"7px 12px", borderRadius:7, border:"1px solid #e5e7eb", background:"#fff", color:"#374151", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Batal</button>
            </div>
          </div>
        )}
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
            ...
            <tbody>
              {users.map((u,i) => {
                ...
                return (
                  <tr key={u.id} style={{ borderBottom:"1px solid #f3f4f6", transition:"background .1s" }}
                  ...
                  >
                    ...
                    <td style={{ padding:"11px 14px" }}>
                      <span style={{ display:"inline-block", padding:"2px 10px", borderRadius:20, fontSize:10, fontWeight:700, background:u.status==="Aktif"?"#dcfce7":"#f3f4f6", color:u.status==="Aktif"?"#15803d":"#6b7280" }}>{u.status}</span>
                    </td>
                    <td style={{ padding:"11px 14px", color:"#6b7280", fontSize:11, whiteSpace:"nowrap" }}>{u.loginTerakhir}</td>
                    <td style={{ padding:"11px 14px", color:"#6b7280", fontSize:11, whiteSpace:"nowrap" }}>{u.dibuat}</td>
                    <td style={{ padding:"11px 14px" }}>
                      <div style={{ display:"flex", gap:6 }}>
                        <button style={{ background:"none", border:"none", cursor:"pointer", color:"#2563eb", padding:3 }} title="Edit"><EditIcon /></button>
                        <button onClick={() => { if(window.confirm("Hapus pengguna ini?")) setUsers(us=>us.filter(x=>x.id!==u.id)); }} style={{ background:"none", border:"none", cursor:"pointer", color:"#dc2626", padding:3 }} title="Hapus"><TrashIcon /></button>
                      </div>
                    </td>
                  </tr>
                );
